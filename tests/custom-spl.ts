...
describe('custom-spl', () => {
  ...
  const program = anchor.workspace.CustomSpl;
  ...
  it("Burns a token", async () => {
    ...
    const toAccount = await getTokenAccount(provider, to);
    const expectedBalance = new anchor.BN(50);
  
    // Use a more flexible assertion that checks if the balance is greater than or equal to the expected value.
    assert.ok(toAccount.amount.gte(expectedBalance), `Expected balance: ${expectedBalance}, Actual balance: ${toAccount.amount}`);
  });
